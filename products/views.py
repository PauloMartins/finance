...
def product(request, options = None):
	total = 0
	items = Product.objects.filter(user_id=request.user).order_by('date').extra(where=['(YEAR(end_date) > %s) OR (YEAR(end_date) = %s AND MONTH(end_date) >= %s)'], params=[datetime.date.today().year, datetime.date.today().year, datetime.date.today().month])
